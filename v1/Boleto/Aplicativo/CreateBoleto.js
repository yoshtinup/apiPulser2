...
export class CreateBoleto {
  ...
  async execute(boletoData) {
    ...
    const { id, tipo, codigo, telefonoTaxi, evento, lugar, url, nombre, status} = boletoData;

    // Crear una instancia de la entidad Boleto con los datos (aplica validaciones si es necesario)
    const boleto = new BoletoUser(id, tipo, codigo, telefonoTaxi, evento, lugar, url, nombre, status);
